...
def salva_todas_as_medidas(diam,grau_medio,num_vert_conect,densidade,betw,bifurc_aa, bifurc_numero_do_no, bifurc_grau_do_no,aa,texto):	

	np.savetxt('Henon/txt/diametro'+texto+'.txt',diam,fmt='%1.2f')
	np.savetxt('Henon/txt/grau_medio'+texto+'.txt',grau_medio,fmt='%1.4f')
	np.savetxt('Henon/txt/num_vert_conect'+texto+'.txt',num_vert_conect,fmt='%1.2f')
	np.savetxt('Henon/txt/densidade'+texto+'.txt',densidade,fmt='%1.5f')
	np.savetxt('Henon/txt/betw'+texto+'.txt',betw,fmt='%1.5f')
	np.savetxt('Henon/txt/aa'+texto+'.txt',aa,fmt='%1.8f')
	np.savetxt('Henon/txt/bifurc_aa'+texto+'.txt',bifurc_aa,fmt='%1.8f')
	np.savetxt('Henon/txt/bifurc_numero_do_no'+texto+'.txt',bifurc_numero_do_no,fmt='%1.2f')
	np.savetxt('Henon/txt/bifurc_grau_do_no'+texto+'.txt',bifurc_grau_do_no,fmt='%1.2f')
...
#dt = 0.01 #time step da integracao
#nTrans = 50000 #usou isso no arquivo que ta carregando
nIt = 4096 
#nTot = nTrans + nIt
nn = 10
...
#constantes de henon
aa = np.linspace(0.0,1.4,1000) #na verdade aqui eh cc, mas vou manter o nome pra nao mudar muita coisa no programa





#para calcular o vetor binario
meio = 0.5 #colocar 0.35 pq da a metade acima e metade abaixo desse valor
texto = '_meio_%s'%(meio)

#metricas das redes
diam = np.zeros(len(aa))
grau_medio = np.zeros(len(aa))
num_vert_conect = np.zeros(len(aa))
densidade = np.zeros(len(aa))
betw = np.zeros(len(aa))
...
#carrega as series temporais dos minimos da coordenada y do rossler
mat = np.loadtxt('../timeseries_generation/henon.txt')
indice1 = 0
indice2 = nIt
...
for i in range(len(aa)):
	...
	mat_adj = mat_adjacencia(vet_decimal,nn,base)
	...
	if (num_vert_conect[i] > 1) : #tem pelo menos um par de vertices conectados
	...
	#betweness sem normalizar
	#betw[i] = np.mean(betw_nohs)
...
titulo_fig = 'Henon/figs/diagrama_bifurc'+texto+'.png' 
plota_diagrama_de_bifurcacao(titulo_fig,bifurc_aa, bifurc_numero_do_no, bifurc_grau_do_no)

titulo_fig = 'Henon/figs/medidas_da_rede'+texto+'.png' 
plota_medidas_da_rede(titulo_fig,diam,grau_medio,num_vert_conect,densidade,betw,aa)


salva_todas_as_medidas(diam,grau_medio,num_vert_conect,densidade,betw,bifurc_aa, bifurc_numero_do_no, bifurc_grau_do_no,aa,texto)	

#nx.draw(grafo, node_color='g', edge_color='k', with_labels=True, alpha=0.5)
